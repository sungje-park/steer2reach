"""
Example script for training/evaluating the 2D vertical drone HJI model.
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Vertical Drone HJI training script")
...
def visualize_results(solver, params, loss_method, run_path=None):
    ...
    z_axis = jnp.linspace(-0.5, 3.5, 120)
    v_axis = jnp.linspace(-4.0, 4.0, 120)
    vv, zz = jnp.meshgrid(v_axis, z_axis)

    x_eval = jnp.stack([zz.reshape(-1), vv.reshape(-1)], axis=-1)
    ...
    u_opt = np.asarray(solver.u_star(x_eval, ux)).reshape(120, 120)

    fig, axes = plt.subplots(2, 2, figsize=(12, 10))

    ax = axes[0, 0]
    im = ax.imshow(v_pred, extent=(-4, 4, -0.5, 3.5), origin="lower", cmap="RdBu", aspect="auto")
    ax.contour(v_axis, z_axis, v_pred, levels=[0.0], colors="black", linewidths=2)
    ax.set_title(f"V(z, vz, t=0) [{loss_method}]")
    ax.set_xlabel("vz")
    ax.set_ylabel("z")
    plt.colorbar(im, ax=ax, label="V")

    ax = axes[0, 1]
    im = ax.imshow(l_val, extent=(-4, 4, -0.5, 3.5), origin="lower", cmap="RdBu", aspect="auto")
    ax.contour(v_axis, z_axis, l_val, levels=[0.0], colors="black", linewidths=2)
    ...
    ax = axes[1, 0]
    im = ax.imshow((v_pred >= 0).astype(float), extent=(-4, 4, -0.5, 3.5), origin="lower", cmap="RdYlGn", aspect="auto")
    ax.contour(v_axis, z_axis, v_pred, levels=[0.0], colors="black", linewidths=2)
    ...
    ax = axes[1, 1]
    im = ax.imshow(u_opt, extent=(-4, 4, -0.5, 3.5), origin="lower", cmap="coolwarm", vmin=-1.0, vmax=1.0, aspect="auto")
    ax.contour(v_axis, z_axis, v_pred, levels=[0.0], colors="black", linewidths=1.5)
    ...
    plt.tight_layout()
    save_path = os.path.join(run_path, f"drone_result_{loss_method}.png") if run_path else f"drone_result_{loss_method}.png"
    plt.savefig(save_path, dpi=150)
    if solver.config.save_to_wandb:
        wandb.summary["drone_plot"] = wandb.Image(fig)
    plt.close(fig)
...
def main():
    ...
    config = VerticalDrone_HJI.get_base_config(traj_len=args.disc)
    ...
    solver = VerticalDrone_HJI(config)
    ...
    if run_name is None:
        ...
        run_name = f"vertical_drone_{args.loss}_{timestamp}"
