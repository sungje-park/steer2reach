"""
Example script for training/evaluating the pursuit-evasion HJI model.
"""
...
from problems import PursuitEvasion_HJI

LOSS_CHOICES = ("vipinns", "pinns", "fspinns", "fspinnsbatched")


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Pursuit-Evasion HJI training script")
    ...
    parser.add_argument("-d", "--disc", type=int, default=50, help="Trajectory discretization length")
    parser.add_argument("-w", "--wandb", type=int, default=0, help="Enable Weights & Biases logging")
    parser.add_argument("-i", "--iter", type=int, default=20000, help="Base stage iterations")
    parser.add_argument("-t", "--test", action="store_true", help="Quick smoke-test mode")
    ...
    parser.add_argument("--batch_pde", type=int, default=2048)
...
def visualize_results(solver, params, loss_method, run_path=None):
    ...
    x_axis = jnp.linspace(-1, 1, 120)
    x1, x2 = jnp.meshgrid(x_axis, x_axis)
    x3 = jnp.ones_like(x1) * (jnp.pi / 2)

    x_eval = jnp.stack([x1.reshape(-1), x2.reshape(-1), x3.reshape(-1)], axis=-1)
    t_eval = jnp.zeros((x_eval.shape[0], 1))

    v_pred = solver.calc_u(params, x_eval, t_eval).reshape(120, 120)
    l_val = solver.l(x_eval).reshape(120, 120)

    fig, axes = plt.subplots(1, 2, figsize=(10, 4))

    ax = axes[0]
    im = ax.imshow(v_pred, extent=(-1, 1, -1, 1), origin="lower", cmap="RdBu")
    ax.contour(x_axis, x_axis, v_pred, levels=[0.0], colors="black", linewidths=2)
    ax.set_title(f"V(x, t=0) [{loss_method}]")
    ax.set_xlabel("x1")
    ax.set_ylabel("x2")
    plt.colorbar(im, ax=ax, label="V")

    ax = axes[1]
    im = ax.imshow((v_pred < 0).astype(float), extent=(-1, 1, -1, 1), origin="lower", cmap="RdYlGn_r")
    ax.contour(x_axis, x_axis, l_val, levels=[0.0], colors="green", linewidths=1.5, linestyles="--")
    ax.contour(x_axis, x_axis, v_pred, levels=[0.0], colors="black", linewidths=2)
    ...
    plt.tight_layout()
    save_path = os.path.join(run_path, f"hji_result_{loss_method}.png") if run_path else f"hji_result_{loss_method}.png"
    plt.savefig(save_path, dpi=150)
    if solver.config.save_to_wandb:
        wandb.summary["hji_plot"] = wandb.Image(fig)
    plt.close(fig)
...
def main():
    ...
    config = PursuitEvasion_HJI.get_base_config(traj_len=args.disc)
    ...
    if run_name is None:
        ...
        run_name = f"pursuit_evasion_{args.loss}_{timestamp}"
